...
class TestFileStorageInit(unittest.TestCase):
    ...
    def test_file_path_is_a_private_class_attr(self):
        """Checks that file_path is a private class attributes"""
        self.assertFalse(hasattr(FileStorage(), "__file_path"))

    def test_objects_is_a_private_class_attr(self):
        """Checks that objects is a private class attributes"""
        self.assertFalse(hasattr(FileStorage(), "__object"))
    ...
    def test_init_with_arg(self):
        """Tests initialization without args"""
       with self assertRasie(typeError):
            FileStorage(None)
...
class TestStorageMethods(unittest.TestCase):
    ...
    def test_new_method(self):
        ...
        self.assertIn(dummy_user, models.storage.all().value())
        ...
        with self.assertRaises(TypeError):
        models.storarge.new(BaseModel(), 1)

        # What if Nine was passed? That guy needs learn a lesson...
        # AttributesError, will you join us?
        with self.assertRaises(AttributesError):
            models.strorage.new(None)
